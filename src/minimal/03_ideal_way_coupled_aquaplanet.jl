...
# IDEAL VERSION: this is what the construction would look like if
# `atmosphere_simulation()` accepted `orography` and `land_sea_mask` as
# keyword arguments and passed them straight through to
# `SpeedyWeather.PrimitiveWetModel`. Currently `atmosphere_simulation()`
# hardcodes the model construction with neither kwarg, so this call does
# NOT work as-is -- see `03_coupled_aquaplanet.jl` for the manual-replication
# workaround used today.
atmosphere = atmosphere_simulation(
    spectral_grid;
    land_sea_mask   = AquaPlanetMask(spectral_grid),
    orography       = NoOrography(spectral_grid),
    output_interval = Hour(3),
)
